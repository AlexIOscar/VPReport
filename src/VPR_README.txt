...
0. Описанная версия - v1.6

Приложение представляет собой единый запускаемый файл с разрешением .jar. Для его запуска необходимо, чтобы
в системе была установлена среда исполнения Java версии не ниже 8.

________________________________________________________________________________________________________________________
1. Назначение

Программа предназначена для генерации и текстового вывода информации, полученной путем анализа данных из сырых
выгрузок, получаемых с оборудования Vernet Behringer.

________________________________________________________________________________________________________________________
2.1. Главное окно. Вкладка "Отчет".

Эта вкладка является первой, и отображается при запуске программы. Основные элементы:

Директория-источник - адрес директории, из которой будут взяты все файлы выгрузок, и далее
                      будет составлена пара отчетов (общий и посменный). Производится захват только файлов
...
Создать отчет - кнопка, запускает создание отчета.

________________________________________________________________________________________________________________________
2.2. Главное окно. Вкладка "Заказ".

Вкладка "отчет" предназначена для поиска и вывода информации по одному конкретному заказу. Поиск осуществляется по
сырым отчетам, размещенным в произвольном множестве директорий.

Источник - область для выбора и отображения всех директорий, по которым будет выполняться поиск данных. Каждая
           директория должна быть размещена в отдельной строке. Кнопка открывает диалог выбора директории, после
           выбора директория будет добавлена к списку. Удаление директорий из списка или полная очистка области
           может быть выполнена обычными манипуляциями с текстом внутри нее.

Заказ - область для внесения обозначения заказа, по которому необходимо найти данные

Вычислить - кнопка, запускающая поиск данных и вывод их в область ниже. В случае, если по размещенному заказу не будет
            найдено ничего, то в области отобразятся нулевые значения величин.
________________________________________________________________________________________________________________________
3.1. Состав отчета (вкладка "Отчет")

Отчет состоит из четырех "подотчетов", с постфиксами "_common", "_shifts", "_pcs" и "_shtbl" в имени файла. Все файлы
отчетов состоят из обычных текстовых данных, первые два сохраняются с расширением .txt, вторые два - с расширение .csv
(и, соответственно, содержат должным образом отформатированный текст, который может быть использован прикладным ПО,
в т.ч. MS Excel или Libre Office, для генерации списков атрибутов).
...
pcs-отчет - подетальный, содержит записи обо всех встреченных в ходе обработки деталях
shtbl-отчет - посменный, содержит детализированную информацию по каждой смене

________________________________________________________________________________________________________________________
3.2. Содержимое результата обработки по заказу (вкладка "Заказ")

Результат представляет собой набор строк с данными. На каждую директорию в списке источников выводится собственный
блок строк, содержащий информацию, собранную по данной директории. В версии v1.6 выводится только два показателя:

Processing time, min:
суммарное время обработки по всем деталям заказа, с примененным коэффициентом (см. "Настройки"), и с учетом откатов
каретки. Время изготовления деталей для нахождения результата подвергается обработке согласно 6.3

Pcs count:
Количество деталей, по которым собрано время, указанное выше.

________________________________________________________________________________________________________________________
4.1 Содержимое отчетов "common" и "shifts" (генерируемых на вкладке "Отчет")
...
________________________________________________________________________________________________________________________
4.2 Содержимое отчета "pcs" (генерируемого на вкладке "Отчет")

Отчет представляет собой текстовый файл, отформатированный согласно CSV (с разделителем ";"). Файлу присваивается
расширение .csv, соответственно, при наличии установленной ассоциации в ОС, он может напрямую открываться средствами
Excel/Libre Office.
Верхняя строка отчета занята заголовками атрибутов, дальнейшие строки представляют собой записи о конкретных деталях.
Каждая деталь, прежде чем быть занесенной в отчет, подвергается обработке своих временных атрибутов (временных меток
начала и конца изготовления, длительности изготовления), с целью ликвидации "наложений" промежутков времени друг на
друга, являющихся неизбежным следствием параллельного исполнения операций машиной (подробнее - 6.3) и фильтрации
через репозиторий (6.1, 6.2).

________________________________________________________________________________________________________________________
4.3 Содержимое отчета "shtbl" (генерируемого на вкладке "Отчет")
Отчет представляет собой текстовый файл, отформатированный согласно CSV (с разделителем ";"). Файлу присваивается
расширение .csv, соответственно, при наличии установленной ассоциации в ОС, он может напрямую открываться средствами
Excel/Libre Office.
Верхняя строка отчета занята заголовками атрибутов, дальнейшие строки представляют собой записи о сменах.
Содержимое атрибутов, в основном, ясно из их имени, конструктивные данные собраны за всю смену (например, общая
обработанная длина, общее количество отверстий, и так далее).
Атрибуты, требующие уточнения:

Машинное время - время, в течении которого оборудование фиксировало выполнение какой-либо детали

Рабочее время - машинное время сложенное со временем откатов каретки, после чего к их сумме применяется коэффициент
                времени (см. 5)

Полное время - время между фиксацией начала производства первой детали и окончанием производства последней, аналогично
               величине "total uptime" из 4.1

Коэфф.загрузки(пв),% - отношение рабочего времени к полному времени, выраженное в процентах (пв - полное время)

Коэфф.загрузки(св),% - отношение рабочего времени к длительности смены (получаемому из настроек, см. 5), выраженное
                       в процентах (св - сменное время)
...
Используется для оценки количества откатов каретки в одном из трех существующих оценочных механизмах. Общий для всех
сортаментов проката.

- Коэффициент (времени)
На эту величину будет умножено время выполнения деталей из выгрузок, прежде чем быть учтенным в расчетах (откаты
каретки учитываются отдельно, и их длительность не увеличивается этим коэффициентом).
В версии v1.6, используется в модуле обработки времени заказов и на "рабочее время" в посменном отчете в CSV-виде
(но никак не влияет на показатели в txt-отчетах).
...
предмет того, встречалась ли она раньше в выгрузках, и если да, то из хранилища берется медианное время по
...
настройки, это новое найденное валидное время для данного изделия может быть помещено в репозиторий. Тогда самое
старое время, которое в нем хранилось раньше, удаляется, но только в том случае, если уже превышен максимальный объем
хранимой для данной детали очереди измерений.
В версии v1.6 длина очереди не может быть настроена пользователем, и составляет 32 измерения
(из-за неуточненных на данный момент особенностей внутренней реализации, репозиторий типа pcRepo.dat (см. 6.2)
фактически использует на одну ячейку меньше - 31).
...
деталь, это новое время попадет в репозиторий, и, поскольку механизм получения медианного времени в случае четного
размера выборки настроен на взятие меньшего, то это первое значение больше не будет возвращаться как результат,
а в конечном итоге оно и вовсе будет вытеснено за пределы очереди.
Все это можно рассматривать как своего рода примитивный процесс обучения репозитория. Его желательно вести на большом
объеме данных, среди которых значительный процент валидных.
...
Категорически фатально для работы указанного механизма начать в ProNC использовать какой-либо из ключевых атрибутов
("марку" или "позицию") для хранения любых быстро меняющихся неинвариантных к конструктиву детали параметров,
например, заказа, партии, или чего-то подобного. В этом случае (что проистекает из описанного выше принципа
работы) репозиторий начнет "дробить" все эти детали на собственные очереди, и вся идея сбора статистики по некоторому
множеству измерений станет неработоспособной.
Теоретически, при таком сценарии, сам репозиторий может со временем разрастись до сколь угодно больших размеров
(поскольку число ключей в нем не ограничено, в отличие от длины хранимых очередей), что может привести к нестабильной
работе (например, зависаниям при старте, завершении, и в процессе работы). Такой "поломанный" репозиторий всегда
можно очистить удалением файла (см. ниже). Новый пустой будет создан на его месте автоматически при следующем
сеансе работы (не следует создавать пустой файл с указанным именем вручную, пустой репозиторий не является пустым
файлом!)

Репозиторий в процессе работы целиком лежит в памяти программы, а между её запусками - сохраняется в файле
C:\Пользователи\Имя_Пользователя\VPRP\pcFastRepo.dat. Ввиду локальности этого файла для конкретной машины, и
динамического характера самого репозитория, на разных машинах данные итоговых отчетов могут быть различными
(если выбран режим фильтрации через репозиторий), ввиду того, что их репозитории "обучались" на разных данных,
и хранят несколько отличающуюся статистику.
В текущей версии, исключить это можно, обучив до "достоверного состояния" какой-то один образец программы, и затем
скопировав её файл pcFastRepo.dat в указанное расположение на другой машине, предварительно отключив во всех
клиентах программы обучение репозитория. В этом случае все отчеты будут одинаковы, однако, новые типы деталей,
если они появятся, будут проходить без фильтрации.

6.2 Дополнительный репозиторий с гарантией уникального вхождения каждой временной метки.

Начиная с версии v1.4, программа использует два репозитория. Второй отличается тем, что, помимо самого времени
выполнения какой-то детали, хранит дополнительно и временную метку, когда эта деталь была изготовлена. Это исключает
возможность многократного влияния одной и той же записи на состояние репозитория. В указанной версии этот репозиторий
существует "параллельно" с обычным fastRepo, и использует все его настройки, но используется он только в анализе
заказов, тогда как fastRepo, как и в прежних версиях, используется для формирования сводных отчетов. Файл репозитория
размещается по адресу C:\Пользователи\Имя_Пользователя\VPRP\pcRepo.dat.

6.3 Обработка записей о деталях для pcs-отчета и отчета по заказу (вкладка "Заказ").
В ситуациях, когда необходимо работать с отдельными деталями (например, для создания подетального отчета, или для
сбора совокупной информации по какому-то множеству номенклатуры), "сырое" время из исходных отчетов машины не может
быть использовано напрямую, поскольку содержит наложения времени друг на друга, в том числе многослойные.
Для ликвидации этой особенности, применяется специальный механизм, который распределяет время между деталями внутри
каждой "группы наложения" таким образом, что наложения ликвидируются, а время распределяется между всеми деталями
пропорционально исходному.
В процессе обработки, исходное временное расположение любой конкретной детали может быть полностью утрачено (вплоть
до порядка детали в группе, ее начального и конечного времени). Гарантируется только, что исходные временные границы
группы будут сохранены (хотя границы изготовления группы могут оказаться занятыми другими деталями, что является
следствием сортировки деталей внутри группы и перераспределением временных меток каждой детали), и почти каждая деталь
(точнее, все, кроме последней в группе, на которую может быть добавлена накопленная ошибка округления) сохранит свой
относительный "вклад" в состав общего времени (с точностью до округления к целому количеству секунд). Ввиду
особенностей этой механики, в редких случаях возникает нулевое время - когда "вклад" детали оказался меньше одной
секунды, он будет округлен до нуля.
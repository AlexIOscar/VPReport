...
Отчет состоит из трех "подотчетов", с постфиксами "_common", "_shifts" и "_pcs" в имени файла. Все файлы отчетов
состоят из обычных текстовых данных, первые два сохраняются с расширением .txt, а отчет pcs - с расширение .csv
(и, соответственно, содержит должным образом отформатированный текст, который может быть использован прикладным ПО,
...
pcs-отчет - общий, содержит записи обо всех встреченных в ходе обработки деталях
...
блок строк, содержащий информацию, собранную по данной директории. В версии v1.5 выводится только два показателя:
...
В версии v1.5, используется только в модуле обработки времени заказов, и никак не сказывается на
работе модуля "Отчеты".
...
равной длине очереди. В версии v1.5 длина очереди не может быть настроена пользователем, и составляет 32 измерения
...
на принимаемое решение должно свестись на нет, пока, в конечном итоге, оно не будет вытеснено за пределы очереди.
Можно это рассматривать, как своего рода примитивный процесс обучения репозитория. Его желательно вести на большом
...
работы) репозиторий начнет "дробить" все эти детали на собственные очереди, и вся идея усреднения станет
неработоспособной. Теоретически, при таком сценарии сам репозиторий может со временем разрастись до сколь угодно
больших размеров (поскольку число ключей в нем не ограничено, в отличие от длины хранимых очередей), что может
привести к нестабильной работе (например, зависаниям при старте, завершении, и в процессе работы). Такой "поломанный"
репозиторий всегда можно очистить удалением файла (см. ниже). Новый пустой будет создан на его месте автоматически
при следующем сеансе работы (не следует создавать пустой файл с указанным именем вручную, пустой репозиторий не
